...
class AccountTree:
    ...
    def _visit_preorder(self, node):
        ...
        if node is None:
            ...
            yield node
            # yield from the left child node
            yield from self._visit_preorder(node.left_child)
            # yield from the right sibling node
            yield from self._visit_preorder(node.right_sibling)
    ...
    def _visit_postorder(self, node):
        ...
        if node is None:
            ...
            # yield from the left child node
            yield from self._visit_postorder(node.left_child)
            # yield from the right sibling node
            yield from self._visit_postorder(node.right_sibling)
            yield node
