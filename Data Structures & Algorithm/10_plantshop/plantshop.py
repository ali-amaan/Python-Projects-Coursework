...
class Plant:
    ...
    def __eq__(self, other):
        """Determine if a plant is equal to other plant by comparing attributes.
    ...
    def __hash__(self):
        """Return a hash for plant instance. Hash of plant is created by calling builtin hash() for
...
class PlantShop():
    def __init__(self, stock=None):
        ...
        """onSale is a set containing plants that are put on sale.
           ...
           discounted with a common discount procent defined below."""
        self.onSale = set()
    ...
    def fillStock(self, plant, quantity=1, price=1):
        """Add plants to stock. If the plant is already found in the stock dict,
        its quantity data is updated and the old price
        is kept. In this case the price parameter is omitted.
        If the plant is a new one, it will be stored as a new key to stock,
        having given quantity and price as its corresponding value tuple."""
        if plant in self.stock:
            new_plant = {plant: (quantity)}
            self.stock.update(new_plant)
        else:
            new_plant = {plant: (quantity, price)}
            self.stock.update(new_plant)
    ...
    def sell(self, plant, pieces=1):
        ...
        if plant in self.stock:
            values = self.stockInfo(plant)
            if plant in self.onSale:
                money = values[1] * (100-self.discount) * pieces / 100
                self.assets = self.assets + money
            else:
                money = values[1] * pieces
                self.assets = self.assets + money
            new_stock = values[0] - pieces
            new_plant = {plant: (new_stock, self.stock[plant][1])}
            self.stock.update(new_plant)
