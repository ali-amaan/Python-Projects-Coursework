...
class RobotGraphicsItem(QtWidgets.QGraphicsPolygonItem):
    ...
    def updatePosition(self):
        ...
        in robot.py.
        """
        y_coord = self.robot.get_location().get_y()
        x_coord = self.robot.get_location().get_x()

        self.setPos(x_coord * self.square_size, y_coord * self.square_size)
    ...
    def updateRotation(self):
        ...
        """
        if self.robot.get_facing() == (0, 1):
            self.setRotation(180)

        if self.robot.get_facing() == (1, 0):
            self.setRotation(90)
