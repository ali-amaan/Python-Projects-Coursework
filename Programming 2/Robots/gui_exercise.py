...
class GuiExercise():
    ...
    def add_robot_world_grid_items(self):
        ...
        for x in range(self.robot_world.get_width()):
            for y in range(self.robot_world.get_height()):
                ...
                rect_item.setPen(QColor(self.dark_gray[0], self.dark_gray[1], self.dark_gray[2]))

                if self.robot_world.squares[x][y].is_wall == True:
                    rect_item.setBrush(QColor(self.dark_gray[0], self.dark_gray[1], self.dark_gray[2]))
                else:
                    rect_item.setBrush(QColor(self.light_gray[0], self.light_gray[1], self.light_gray[2]))
    ...
    def add_robot_graphics_items(self):
        ...
        for robot in self.robot_world.get_robots():
            if robot in self.added_robots:
                continue
            else:
                robot_instance = RobotGraphicsItem(robot, self.square_size)
                self.added_robots.append(robot)
                self.scene.addItem(robot_instance)